require "application_system_test_case"

class Intranet::CidadesTest < ApplicationSystemTestCase
  setup do
    @intranet_cidade = intranet_cidades(:one)
  end

  test "visiting the index" do
    visit intranet_cidades_url
    assert_selector "h1", text: "Intranet/Cidades"
  end

  test "creating a Cidade" do
    visit intranet_cidades_url
    click_on "New Intranet/Cidade"

    fill_in "Descricao", with: @intranet_cidade.descricao
    fill_in "Estado", with: @intranet_cidade.estado
    click_on "Create Cidade"

    assert_text "Cidade was successfully created"
    click_on "Back"
  end

  test "updating a Cidade" do
    visit intranet_cidades_url
    click_on "Edit", match: :first

    fill_in "Descricao", with: @intranet_cidade.descricao
    fill_in "Estado", with: @intranet_cidade.estado
    click_on "Update Cidade"

    assert_text "Cidade was successfully updated"
    click_on "Back"
  end

  test "destroying a Cidade" do
    visit intranet_cidades_url
    page.accept_confirm do
      click_on "Destroy", match: :first
    end

    assert_text "Cidade was successfully destroyed"
  end
end
