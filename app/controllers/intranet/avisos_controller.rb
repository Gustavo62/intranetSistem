class Intranet::AvisosController < ApplicationController
	...
	def index
		if user_signed_in? 
			@intranet_avisos = Intranet::Aviso.where(ativo: true, recipient_id: current_user.id,master: false).order(:created_at)
	...
	def create 
		...
		respond_to do |format|
			if @intranet_aviso.save
				DispararEmailAvisoJob.perform_later(@intranet_aviso)
